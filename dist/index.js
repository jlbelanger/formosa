...
function Autocomplete(_ref) {
  ...
  const [optionValues, setOptionValues] = React.useState(options ? normalizeOptions(options, labelKey, valueKey) : []);
  const [isLoading, setIsLoading] = React.useState(showLoading || !!url);
...
function File(_ref) {
  ...
  const [filenames, setFilenames] = React.useState(getFilenames(currentValue));
  const [srcs, setSrcs] = React.useState(getSrcs(currentValue));
  React.useEffect(() => {
...
function Password(_ref) {
  ...
  const [tempType, setTempType] = React.useState('password');
...
function FormInner(_ref) {
  ...
  const {
    addToast
  } = React.useContext(FormosaContext);
...
function Spinner(_ref) {
  ...
  const {
    promiseInProgress
  } = reactPromiseTracker.usePromiseTracker();
...
function FormContainer(_ref) {
  ...
  const [showWarningPrompt, setShowWarningPrompt] = React.useState(true);
